...
| `executesql-pattern.md` | Resilient ExecuteSQL pattern using SQL.GetFieldName() and SQL.GetTableName() to dynamically resolve names; prevents breakage on field/table renames; Let/Substitute template with placeholders; anti-patterns | ExecuteSQL, SQL, SQL.GetFieldName, SQL.GetTableName, SQL.GetColumn, SQL.GetRecordsAsJSON, query, SELECT, WHERE, Substitute |
