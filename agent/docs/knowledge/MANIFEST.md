...
| `go-to-layout-by-calc.md` | `Go to Layout [SelectedLayout]` sin elemento `<Layout>` no navega → server-side `New Record` da error **106 "Table is missing"** y `Perform Find` devuelve **found 0** (aunque ExecuteSQL sí lea); client-side a veces lo tapa. Las 3 formas de `Go to Layout` (lista `<Layout id/name>`, cálculo `LayoutNameByCalc` + `<Layout><Calculation>`, y Original/Current). Bug del conversor `fm_xml_to_snippet.py` que omitía el destino por cálculo (corregido). Diagnóstico: si funciona en cliente pero falla server-side, sospechar del Go to Layout. | Go to Layout, LayoutDestination, SelectedLayout, LayoutNameByCalc, Layout, Calculation, 106, table missing, found 0, Perform Find, New Record, server-side, Data API, OData, bridge, fm_xml_to_snippet, conversor, OriginalLayout |
