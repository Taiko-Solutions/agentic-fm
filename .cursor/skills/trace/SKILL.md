...
> **(Taiko) MCP first.** If the `agentic-fm-app` MCP is available, answer usage and impact questions with it instead of building `xref.index`: `fm_refs` (direction `in`, IDs + line/location; `depth` follows script calls) for "where is X used", `fm_impact` for "what breaks", plus `fm_search_code` with the object name to catch by-name uses inside calculations (ExecuteSQL, `GetFieldName`, JSON keys, URLs), which the index does not resolve yet. For a field, report every `fm_search_code` hit written as `TO::Field` whose TO is based on the field's table (check with `fm_find` kind `table_occurrence`) as a **text use** in its own section — do not drop it because `fm_refs` did not list it; only discard hits whose TO belongs to another table. Fall back to `trace.py` below for dead-object scans, when the MCP is not registered, or when a call fails. Dead-object scans → `fm_unused kind=<kind> solution=<file>` (it separates name-only mentions; entry points outside the index are listed as a caveat).
