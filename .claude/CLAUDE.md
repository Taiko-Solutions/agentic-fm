...
8. Once validation passes, inform the user the script is ready in `agent/sandbox/<script_name>`. Only run `clipboard.py write` or `deploy.py` if the user explicitly requests it (see Clipboard section above). When falling back to Tier 1 (manual paste), present instructions in this exact format:
